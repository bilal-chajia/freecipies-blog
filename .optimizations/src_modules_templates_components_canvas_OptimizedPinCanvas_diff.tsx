// @ts-nocheck
//-- src/modules/templates/components/canvas/OptimizedPinCanvas.tsx (原始)


+++ src / modules / templates / components / canvas / OptimizedPinCanvas.tsx(修改后)
// @ts-nocheck
...
import { useKeyboardShortcuts, useImageLoader, getProxiedUrl } from './hooks';
...
/**
 * OptimizedPinCanvas - High-performance Konva-based canvas for Pinterest pin design
 * Features: Transformer controls, zoom, grid, optimized rendering, premium UI
 * Optimizations: Layer separation, uncontrolled drag updates, image caching
 */
const OptimizedPinCanvas = ({
    template = null,
    articleData = null,
    onExport = null,
    editable = true,
    ...
    // Handle drag move with optimized smart guides
    const handleDragMove = useCallback((e) => {
        ...
        // --- Optimized Smart Snapping Logic ---
        // Only consider canvas edges and center as guide lines
        const guideLines = {
    ...
    // Handle drag end with optimized approach
    const handleDragEnd = useCallback((id, e) => {
        ...
        // Only update React state on drag end, not during drag
        handleElementChange(id, {
            x: e.target.x(),
            y: e.target.y(),
        });
    ...
    const wrappedHandleDragMove = (e) => {
        // Only update visual position during drag, don't trigger React re-renders
        ...
        // Update guides but don't trigger React state updates during drag
        // This is handled in handleDragMove
        handleDragMove(e);
    };
    ...
    const renderElement = (element) => {
        ...
        const commonProps = {
            ...
            onDragStart: isLocked ? undefined : (e) => handleDragStart(element.id, e),
    ...
    // Render the canvas with optimized layer structure
    return (
        <div
            ref={containerRef}
        ...
        >
            ...
            >
                {/* Background layer - static elements, no interactivity */}
...
export { OptimizedPinCanvas, DEFAULT_CANVAS_WIDTH, DEFAULT_CANVAS_HEIGHT, GRID_SIZE };
export default OptimizedPinCanvas;