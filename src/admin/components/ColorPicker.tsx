...
const ColorPicker = ({
  ...
  const parseColor = (c?: string): ParsedColor => {
  ...
  const outputColor = useCallback((r: number, g: number, b: number, a: number) => {
  ...
  const updateFromHsv = useCallback((newHsv: HSV, newOpacity = opacity) => {
  ...
  const handleOpacityMove = (e: React.MouseEvent | MouseEvent) => {
    const target = (e.target as HTMLElement).closest('[data-opacity]')?.parentElement || opacityRef.current;
    if (!target) return;
