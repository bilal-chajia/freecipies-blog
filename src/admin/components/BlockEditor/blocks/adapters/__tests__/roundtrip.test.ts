...
describe('BlockAdapter round-trip: DB → Editor → DB', () => {
  const testCases: {
    ...
    },
    {
      name: 'video',
      adapterType: 'video',
      db: { type: 'video', provider: 'youtube', videoId: 'abc123', aspectRatio: '16:9' },
    },
    {
      name: 'divider',
      adapterType: 'divider',
    ...
    },
    {
      ...
      db: {
        type: 'faq_section',
        title: 'FAQ',
        items: [{ q: 'Can I freeze it?', a: 'Yes, up to 3 months' }],
      },
    },
    {
      ...
      db: {
        ...
        layout: 'grid',
        mode: 'manual',
        recipes: [{ id: 1, title: 'Test Recipe', slug: 'test-recipe' }],
      },
    },
    {
      name: 'before_after',
...
/**
 * MainRecipeAdapter uses a class-based interface (blockType, toBlockNote,
 * fromBlockNote) that differs from the standard BlockAdapter interface
 * (type, toEditor, fromEditor). It is tested separately here.
 */
describe('MainRecipeAdapter round-trip (class-based adapter)', () => {
  const adapter = new MainRecipeAdapter();

  it('is registered and has correct blockType', () => {
    expect(adapter.blockType).toBe('mainRecipe');
  });

  it('round-trips a recipe block via toBlockNote / fromBlockNote', () => {
    const dbBlock = {
      type: 'mainRecipe',
      recipe: {
        id: 1,
    ...
    // DB → Editor
    const editorBlock = adapter.toBlockNote(dbBlock);
    expect(editorBlock.type).toBe('mainRecipe');
    expect(editorBlock.props).toBeDefined();

    // Editor → DB
    const backToDb = adapter.fromBlockNote(editorBlock as any);
    expect(backToDb.type).toBe('mainRecipe');
    expect(backToDb.recipe).toBeDefined();
  });
});
