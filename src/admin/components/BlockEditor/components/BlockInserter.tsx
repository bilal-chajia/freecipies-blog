...
import { useStructureTree } from './structure/useStructureTree';
...
export default function BlockInserter({
    ...
    const [panelTab, setPanelTab] = useState<'blocks' | 'list' | 'outline'>('blocks');
    const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>(
        blockCategories.reduce((acc, cat) => ({ ...acc, [cat.id]: true }), {})
    );

    // Use structure tree hook for list/outline views
    const { structureItems: treeItems, outlineItems, visibleItems } = useStructureTree(
        blocks,
        panelTab
    );
