...
function RoundupListSettings({
  ...
  const image = getImageSlot(item.imagesJson, 'thumbnail') || getImageSlot(item.imagesJson, 'hero');
        ...
        // Snapshot stats
        const stats = {
            totalTime: item.totalTimeMinutes || null,
            difficulty: item.difficultyLabel || null,
            rating: null, // Would come from cached_rating_json in a real scenario
        };

        if (item.cachedRatingJson) {
            try {
                const rating = JSON.parse(item.cachedRatingJson);
                stats.rating = rating.ratingValue;
            } catch {}
        }
    ...
    const addItem = (item) => {
        if (!item?.id) return;
        if (items.some((existing) => existing.articleId === item.id)) return;
            ...
            >
                ...
                {results.length > 0 && (
                    <div className="border rounded-xl bg-muted/30 overflow-hidden divide-y max-h-48 overflow-y-auto">
                        {results.map((item) => {
                            const isAdded = items.some((ex) => ex.articleId === item.id);
                            return (
                        ...
                        <div className="space-y-2">
                            {items.map((item, index) => (
                                <Collapsible 
                                    ...
                                    open={openItemId === item.articleId}
                                    onOpenChange={(open) => setOpenItemId(open ? item.articleId : null)}
                                    ...
                                    <div className="p-2 flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-0.5">
                                                ...
                                                {item.stats?.totalTime && (
                                                    <span className="text-[9px] text-muted-foreground">· {item.stats.totalTime}m</span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center shrink-0">
                                            ...
                                            <Button 
                                                ...
                                                onClick={(e) => { e.stopPropagation(); removeItem(item.articleId); }}
                                            >
