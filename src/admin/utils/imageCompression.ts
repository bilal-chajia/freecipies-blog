 ...
 */
const calculateDimensions = (width: number, height: number, maxWidth: number, maxHeight: number): Dimensions => {
    let newWidth = width;
 ...
 * @returns Compressed file and stats
 */
export async function compressImage(file: File, options: CompressionOptions = {}): Promise<{ file: File; stats: CompressionStats }> {
    const {
