...
import useEditorStore, { type EditorElement, type TextElement, type ImageSlotElement } from '@admin/features/templates/store/useEditorStore';
...
const ContextToolbar: React.FC = () => {
    ...
    const shapeColorTriggerRef = useRef<HTMLDivElement>(null);
    const fontInputRef = useRef<HTMLInputElement>(null);

    // Get the bounding rect of a ref element
    const getTriggerRect = (ref: React.RefObject<HTMLDivElement | null>): DOMRect | null => {
        if (ref.current) {
    ...
    const updateProp = (key: string, value: unknown) => {
        ...
        updateElement(selectedElement.id, { [key]: value } as Partial<EditorElement>);
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="relative">
                        ...
                        onChange={(e) => {
                            const file = e.target.files?.[0];
                            if (file) {
                                toast.error('Template image replacement needs the dedicated template asset flow first.');
                            }
                            e.target.value = '';
                        }}
                        />
