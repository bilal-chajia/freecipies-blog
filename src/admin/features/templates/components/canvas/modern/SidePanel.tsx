...
const getTemplateIdentity = (template: TemplateState | null | undefined) => template?.id ?? template?.slug ?? null;

const upsertTemplate = (templates: TemplateState[], savedTemplate: TemplateState) => {
    const savedIdentity = getTemplateIdentity(savedTemplate);
    let didUpdate = false;

    const nextTemplates = templates.map((template) => {
        const sameId = savedTemplate.id !== undefined && template.id === savedTemplate.id;
        const sameSlug = savedTemplate.slug !== undefined && template.slug === savedTemplate.slug;
...
const SidePanel: React.FC = () => {
    ...
    const bgColorTriggerRef = useRef<HTMLDivElement>(null);
    ...
    // Template-specific asset upload is intentionally disabled until the
    // dedicated template asset route exists. Do not write canvas assets through
    // the editorial media upload flow.
    ...
    const executeDeleteTemplate = async (templateSlug: string) => {
        try {
    ...
    interface ConfirmState {
        isOpen: boolean;
        type: 'delete_template' | 'unsaved_changes' | null;
        data: unknown;
        title: string;
        description: string;
        confirmText?: string;
    ...
    const handleConfirmAction = () => {
        if (confirmState.type === 'delete_template') {
            const templateSlug = confirmState.data as string;
            executeDeleteTemplate(templateSlug);
        } else if (confirmState.type === 'unsaved_changes') {
            const template = confirmState.data as TemplateState;
    ...
    const renderContent = () => {
        ...
        const handleTemplateClick = (t: TemplateState) => {
        ...
        const handleDeleteTemplate = async (e: React.MouseEvent, templateSlug: string) => {
            e.stopPropagation();
            setConfirmState({
                isOpen: true,
        ...
        const handleDuplicateTemplate = async (e: React.MouseEvent, templateToCopy: TemplateState) => {
            e.stopPropagation();
            try {
                // Create unique slug using centralized utility
                const newSlug = cleanDuplicateSlug(templateToCopy.slug || '');

                const newTemplateData: Record<string, unknown> = {
                    ...templateToCopy,
                            ...
                            {isLoadingTemplates ? (
                                ...
                                templates.map((t, index) => (
                                    ...
                                    >
                                        ...
                                        {t.thumbnail_url ? (
                                            <img
                                                key={`${t.id || t.slug}-${t.thumbnail_url}`}
                                                src={t.thumbnail_url}
                                                alt={t.name}
                        ...
                        <div className="space-y-2 relative">
                            ...
                            {showBgColorPicker && (
                                <ColorPicker
                                    ...
                                    onChange={(color) => setTemplate({ background_color: color ?? '#ffffff' })}
