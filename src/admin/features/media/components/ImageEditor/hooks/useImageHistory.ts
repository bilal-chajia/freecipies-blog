 ...
 * @param currentStateGetter - Function that returns the current state snapshot
 * @returns History management functions and state
 */
export const useImageHistory = <T>(currentStateGetter: () => T) => {
    ...
    const getPreviousState = useCallback((): T | null => {
        if (historyIndex > 0) {
    ...
    const getNextState = useCallback((): T | null => {
