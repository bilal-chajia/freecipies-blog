...
import type { SortableMenuItemRowProps } from '../../types/menu-editor.types';

function SortableMenuItemRow({ item, isSelected, onClick, onDelete }: SortableMenuItemRowProps) {
    const {
        attributes,
        listeners,
        setNodeRef,
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <span className="truncate max-w-[200px]">{item.target?.href || item.overview_target?.href || '#'}</span>
                    {item.type === 'mega' && (
                        <>
