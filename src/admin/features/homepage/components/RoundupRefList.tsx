...
import { mapArticleToRoundupRef, addRoundupRef, buildPickerSearchParams } from '../utils/ref-mappers';
...
function RefRow({ ref: refItem, onRemove }: RefRowProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 p-2 rounded-sm border border-border bg-muted/40">
      <button type="button" aria-label={`Reorder ${refItem.title}`} className="cursor-grab touch-none text-muted-foreground hover:text-foreground active:cursor-grabbing" {...attributes} {...listeners}>
...
const RoundupRefList: React.FC<RoundupRefListProps> = ({ refs, onChange }) => {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    setIsSearching(true);
    try {
      const res = await api.get('/articles', { params: buildPickerSearchParams('roundup', query) });
      const data = res.data;
