 ...
 */
export async function getRedirectByFromPath(db: D1Database, fromPath: string): Promise<Redirect | null> {
  const drizzle = createDb(db);
 ...
 */
export async function getRedirectById(db: D1Database, id: number): Promise<Redirect | null> {
  const drizzle = createDb(db);
  const result = await drizzle
    .select()
    .from(redirects)
 ...
 */
export async function createRedirect(db: D1Database, data: NewRedirect): Promise<Redirect> {
  const drizzle = createDb(db);
 ...
 */
export async function deleteRedirect(db: D1Database, id: number): Promise<boolean> {
  const drizzle = createDb(db);
