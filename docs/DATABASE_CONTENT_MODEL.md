...
- `docs/CONTENT_BLOCKS_CONTRACT.md`: block vocabulary stored inside `articles.content_json.blocks`.
- `docs/RECIPE_JSON_CONTRACT.md`: only `articles.recipe_json`.
- `docs/ROUNDUP_JSON_CONTRACT.md`: only `articles.roundup_json`.
...
- SEO/content caches: `jsonld_json`, `cached_toc_json`
- listing/render caches: `cached_tags_json`, `cached_category_json`, `cached_author_json`, `cached_rating_json`, `cached_recipe_json`, `cached_card_json`
- indexed scalar mirrors: `reading_time_minutes`
...
- article source JSON fields: `content_json`, `recipe_json`, `roundup_json`, `images_json`
...
The BlockEditor owns `content_json` editing only. Recipe data, article images, SEO/config, and tags are adjacent article editing concerns.

### Public Astro Frontend

Public pages read articles through module services, then render:

- `content_json` through `ContentRenderer.astro`
- `recipe_json` in full recipe layouts/components
- `images_json` for hero, thumbnail, and content imagery
...
- Catalog equipment render snapshots are copied into `recipe_json.equipment[]`
  when a recipe saves.
- SQL triggers should not rebuild equipment snapshots because the payload
  depends on recipe-specific fields, active equipment rows, affiliate metadata,
